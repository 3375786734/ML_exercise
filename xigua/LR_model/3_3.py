"""
这个程序可以学习一些运算操作+文件读取操作,虽然最后因为d2beta不可逆而无法使用
"""
import numpy as np
#linalg是线性代数模块，包括求范数
...
#numpy array用于构造数组。read_excel读取出的数据是元组结构tuple。列表可读写,元组只读。list用于把元组转换成列表，也就是把表格数据打散。
#单引号指定某一列,头部的标签
...
#注意这里为什么要[[],[],[]]是因为这里定义的是一个矩阵,那么每一行一个列表,装置之后变成[[a,b,c]]取出矩阵的第一行因此需要[0]在后面
beta=np.array([[0],[0],[1]])
#3.27式的l(β）,是上一次迭代的l值
old_l=0
#n是迭代次数
